...
# ╔═╡ 38228e4a-19cd-11ee-1d21-af0d5c13acc0
using Turing, Distributions, FillArrays

# ╔═╡ 73ee7b75-8139-42a2-9055-c15c3756f78b
using LinearAlgebra, StatsFuns

# ╔═╡ 2de7c200-bb40-4eb1-9f17-846b8a478722
using Distances

# ╔═╡ 7cbbd876-767a-4e48-9645-7bbca46469f8
using PlutoUI, StatsPlots

# ╔═╡ 2371c343-e9a4-4f78-9abc-4cafc505b6a6
using Zygote

# ╔═╡ a1f28a0e-1df7-4ef1-9e74-5dab4c14823a
using Statistics

# ╔═╡ 5765e955-968e-40ed-82d4-d2a4bd7046f4
hill(x; K, n) = (x.^n)./(K.^n + x.^n)

# ╔═╡ 8be3e3ed-a83e-4995-898b-b2e1a7eed598
begin
	N = 52
	X = abs.([rand(Normal(10+i/(N/52), 20)) for i in 1:N])
	X[6:20] .= 0
	c = hcat(rand(Normal(0, 1), (N, 5)), 1:N)
	error = rand(Normal(0, 0.05), N)
	βs = [1.0; -1; .2; .1; .2; 1]
	β = 1
end;

# ╔═╡ cd4344a1-6409-4fbb-af50-ccb6cd9b9af2
my_k = @bind k Slider(10.0:80.0)

# ╔═╡ c7f46ce1-c024-40b1-97d3-7b97d92b4f99
my_n = @bind n Slider(.5:.1:3.0)

# ╔═╡ 473d51ec-ce91-49a8-ac33-f15bab15d500
md"""
## K = $(k), n = $(n)
"""

# ╔═╡ 1650df41-d465-47b3-9235-070240ef54d4
hill_prime(x; K, n) = n .* x .^ (n .- 1) ./ (K .^ n + x .^ n) .* (1.0 .- hill(x, K=K, n=n))

# ╔═╡ 593bcaf3-78d7-40ef-8146-bc615bc614cd
function hill_double_prime(x; K, n)
	X_n = x .^ n
	K_n = K .^ n
	M = (K_n .+ X_n)
	L = 2.0 .* n .^ 2 .* x .^ (2.0 .* n .- 2) ./ (M .^ 2)
	L2 = X_n .* ((2.0 .* n .^ 2.0 .* x .^ (2.0 .* n .- 2.0)) ./ (M .^ 3) .- ((n-1) .* n .* x .^ (n .- 2.0)) ./ (M .^ 2.0))
	L3 = ((n .- 1) .* n .* x .^ (n .- 2.0)) ./ M
	return -L .+ L2 .+ L3
end	

# ╔═╡ 438dbc4a-c4c5-4da4-9705-124214a67da4
g(x) = hill(x, K=30, n=2)

# ╔═╡ ae82f86e-c255-4877-956f-093b1c1ad2c2
π

# ╔═╡ 5d69e85e-861c-4ff6-b3d0-c0628c56f3da
function hill_max(K, n)
	return exp.(1.0im*π ./ n) .* Complex.( -n .^2 .* K.^n .- n .* K .^ n).^(-1.0 ./ n) .* Complex.(n .^ 2.0 .* K .^ (2.0 .* n) .- n .* K .^ (2.0 .* n)) .^ (1.0 ./ n)
end

# ╔═╡ 26ceb782-b0a9-40c2-a23d-fd6cc1996fda
real.(hill_max(30, 2))

# ╔═╡ c2924b5c-6ea2-4850-b0e9-a600759aa974
let
	f(x) = g(x)
	plot(f, 0, 100)
	plot(x -> hill_double_prime(x, K=50, n=2), 0, 100)
	plot!([50, 50], [-.001, .0025])
end

# ╔═╡ 7689a99c-3123-4037-a263-ebfc5b2b9408
begin
	X_hill = hill.(X, K=k, n=n)
	y = (c * βs
  		+ β*X_hill .+ 5 
  		+ error)
end;

# ╔═╡ 17135459-5ea7-4923-a8ef-0185ca12b70c
scatter(X, X_hill)

# ╔═╡ d72bf987-fbe1-4eb1-9f80-0436a06e3a24
logistic(x) = exp(x)/(1+exp(x))

# ╔═╡ bc0d7f26-a664-4137-bdf8-5128d3f94c80
plot(logistic, -5, 5)

# ╔═╡ 9046fda7-26f2-4e99-a8d1-81dfd620453c
s_curve(x; alpha=.95, beta=.001) = beta.^(alpha.^x) .- beta

# ╔═╡ 1b651e4b-b40a-46f7-ace9-d1aa4ba52ea7
@model GPRegression(y, X, c) = begin
  # Priors.
  n ~ TruncatedNormal(1, 2, .5, Inf)
  K ~ TruncatedNormal(20, 40, 1, Inf)
  
  # Covariance function.
  x_t = hill.(X, n=n, K=K)


  βc ~ filldist(Normal(0, 1), size(c)[2])
  α ~ Normal(0, 1)
  error ~ TruncatedNormal(0, .3, 1e-5, Inf)
  β ~ TruncatedNormal(0, 1, 0, Inf)
  y ~ MvNormal(c * βc + β*x_t .+ α, error^2*LinearAlgebra.I)
  return y
end;

# ╔═╡ 3a845a23-ae39-480a-b37f-6f97b06a6ee2
begin
	model = GPRegression(y, X, c)
	model_samples = sample(model, NUTS(), 1000)
	describe(model_samples)
end

# ╔═╡ 0ec730aa-d38d-4243-876d-74d36b461ec0
let
	values = zeros(1000, 120)
	for i in 1:120
  		values[:, i] = hill_prime.(i, K=model_samples[:K], n=model_samples[:n])	
	end
	
	values = values.* model_samples[:β].data
	q05 = zeros(120)
	q95 = zeros(120)
	for i in 1:120
		q05[i] = quantile(values[:, i], .025)
		q95[i] = quantile(values[:, i], .975)
	end
	
	p = plot(1:120, q05, label="5% CI", c=:black, ls=:dash)
	plot!(1:120, q95, label="95% CI", c=:black, ls=:dash)
	plot!(1:120, mean(values', dims=2), label="mean", c=:red)
	plot!(1:120, hill_prime.(1:120, K=k, n=n)*β, label="true", c=:blue)
	xlims!(0, 120)
end

# ╔═╡ 5e847bd6-52c1-44a4-87cb-8f1b076ba37b
maxs = map(real, filter(x->(imag(x)<1e-5) & (real(x)>0), (hill_max.(model_samples[:K], model_samples[:n]))))

# ╔═╡ 1ab5bfe4-32ac-4d2c-8fa8-f05474b2bb39
let

	values = zeros(1000, 120)
	for i in 1:120
  		values[:, i] = hill.(i, K=model_samples[:K], n=model_samples[:n])	
	end
	...
	maxq05 = quantile(maxs, .025)
	maxq95 = quantile(maxs, .975)
	p = plot(1:120, q05, label="5% CI", c=:black, ls=:dash)
	plot!(1:120, q95, label="95% CI", c=:black, ls=:dash)
	plot!(1:120, mean(values', dims=2), label="mean", c=:red)
	plot!(1:120, hill.(1:120, K=k, n=n)*β, label="true", c=:blue)
	scatter!([real(hill_max(k, n))], [0], c=:black, label="optimal", alpha=.5)
	plot!([maxq05, maxq95], [0.0, 0.0], c=:black, label="")
	scatter!([maxq05, maxq95], [0.0, 0.0], label="estimated optimal", alpha=.5)
	scatter!([mean(maxs)], [0.0], c=:red, label="", alpha=.5)
	xlims!(0, 120)
	h = histogram(X, bins=Int(floor(sqrt(N))), label="")
	xlims!(0, 120)
	plot(p, h, layout = @layout [a{.7h}; b])
end

# ╔═╡ a90ab5a3-6ee8-4e9e-a50d-10c8e39f4b97
let
	values = zeros(1000, N)
	for i in 1:N
  		values[:, i] = hill.(X[i], K=model_samples[:K], n=model_samples[:n]) 
	end
	values = values.* model_samples[:β].data
	contributions_sampled = sum(values, dims=2)
	contributions = sum(hill.(X, K=k, n=n)*β)
	boxplot(contributions_sampled, label="Estimated Contributions")
	scatter!([1.0], [contributions], label="Contributions")
end

# ╔═╡ 77e7c451-5de8-4bf0-a6b2-59c9f73ef4e4
model_priors = sample(model, Prior(), 1000)

# ╔═╡ b682150f-ca43-4100-8042-c75b1e74806b
test_model = GPRegression(missing, X, c)

# ╔═╡ f4e4708c-9724-4e28-a041-b32d0f698ea0
test_samples = sample(test_model, Prior(), 1000)

# ╔═╡ 4b01989d-25c9-4aa4-ad4b-e15d3f9d87a3
let
	values = zeros(N, 1000)
	for i in 1:N
		values[i, :] = test_samples[Symbol("y[$(i)]")]
	end
	plot(1:N, values, color=:black, alpha=.01, label="")
	plot!(1:N, y, color=:blue)
	
end

# ╔═╡ f94e2a08-8dee-4a13-a0d2-f808f6af8fcb
scatter(X, y-c[:, 6])
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "081640e40425a3176677dd50baba2c8e6f8d1a60"
